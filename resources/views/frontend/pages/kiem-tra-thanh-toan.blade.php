...
<style>
    ...
    .failure-icon { background: #f44336; }
    .status-title {
        font-size: 24px;
        font-weight: bold;
        margin-bottom: 8px;
    }
    .status-title.success { color: #333; }
    .status-title.failure { color: #f44336; }
    .status-subtitle { color: #666; font-size: 14px; margin-bottom: 10px;}
    .error-message {
        background: #ffebee;
    ...
    .movie-card.failure { border: 2px solid #ffcdd2; opacity: 0.7;}
    .ticket-header {
        background: #f9fafb;
        margin: 0;
        padding: 20px 30px;
    ...
    .ticket-header.failure { background: #ffebee; border-bottom: 2px dashed #ffcdd2;}
    ...
    .ticket-header.failure::after { border: 2px solid #ffcdd2;}
    ...
    .movie-title.failure { background: #757575; }
    .ticket-content { padding: 25px 30px; position: relative; z-index: 2;}
    .movie-info {
        display: grid;
    ...
    .cinema-section.failure { border-top: 2px dashed #ffcdd2;}
    .cinema-name {
        font-size: 16px;
        font-weight: bold;
        margin-bottom: 10px;
    ...
    .cinema-name.failure { color: #757575;}
    .cinema-address {
        font-size: 13px;
        color: #6b7280;
        line-height: 1.6;
    ...
    .cinema-address.failure { color: #9e9e9e;}
    .cinema-details {
        display: flex;
    ...
    .cinema-room.failure, .cinema-format.failure {
        background: #f5f5f5;
        color: #757575;
    ...
    .ticket-number.failure { color: #c62828;}
    .pricing-section {
        background: #f9f9f9;
        padding: 20px;
        border-radius: 8px;
    ...
    .pricing-label.failure { color: #757575;}
    .pricing-value { font-weight: bold; color: #333;}
    .pricing-value.failure { color: #757575;}
    .seats-row {
        border-top: 1px solid #eee;
        padding-top: 15px;
        margin-top: 15px;
    }
    .seats-row.failure { border-top: 1px solid #ffcdd2;}
    .total-section {
        background: white;
        padding: 15px 20px;
        border-top: 1px solid #eee;
    ...
    .total-label.failure { color: #757575;}
    .total-amount {
        font-size: 18px;
        font-weight: bold;
        color: #333;
    }
    .total-amount.failure { color: #757575; text-decoration: line-through;}
    .qr-section {
        width: 400px;
    ...
    .contact-button:hover { background: rgba(255,255,255,0.1);}
    .action-buttons {
        display: flex;
        gap: 15px;
        justify-content: center;
    ...
    .main-button:hover { background: #d32f2f; transform:translateY(-1px);}
    .secondary-button {
        background: transparent;
        color: #f44336;
    ...
    .secondary-button:hover { background: #f44336; color: white;}
    @media (max-width: 768px) {
        .content-wrapper { flex-direction: column;}
        .qr-section, .failure-section { width:100%; }
        .action-buttons { flex-direction: column; align-items: center;}
        .main-button, .secondary-button { width: 80%; max-width: 300px;}
    }
</style>
<div class="container-payment-status">
    ...
    <div class="content-wrapper">
            ...
            <div class="movie-card @if(session('status') !== 'success') failure @endif">
                ...
                <div class="ticket-content">
                    <div class="movie-info">
                        <div class="info-item">
                            <div class="movie-info-label @if(session('status') !== 'success') failure @endif">Thời gian</div>
                        ...
                        <div class="info-item">
                            <div class="movie-info-label @if(session('status') !== 'success') failure @endif">Ngày chiếu</div>
                            <div class="movie-info-value @if(session('status') !== 'success') failure @endif">
                    ...
                    <div class="cinema-section @if(session('status') !== 'success') failure @endif">
                        ...
                        <div class="cinema-details">
                            <div class="info-item">
                                <div class="movie-info-label @if(session('status') !== 'success') failure @endif">Phòng chiếu</div>
                                <div class="cinema-room @if(session('status') !== 'success') failure @endif">
                                    {{ $suatChieu->phongChieu->TenPhongChieu ?? $cinemaRoom ?? '---' }}
                                </div>
                            </div>
                            <div class="info-item">
                                <div class="movie-info-label @if(session('status') !== 'success') failure @endif">Ghế</div>
                            ...
                            <div class="info-item">
                                <div class="movie-info-label @if(session('status') !== 'success') failure @endif">Đồ họa</div>
                                <div class="cinema-format @if(session('status') !== 'success') failure @endif">
            ...
            <div class="pricing-section @if(session('status') !== 'success') failure @endif">
                ...
                <div class="seats-row @if(session('status') !== 'success') failure @endif">
                    ...
                    <div class="pricing-row">
                        ...
                        <span class="pricing-value @if(session('status') !== 'success') failure @endif">0đ</span>
                    </div>
                    <div class="pricing-row">
        ...
        <div class="qr-section">
            ...
            <div class="qr-text">Vui lòng cung cấp mã QR Code này cho nhân viên tại rạp. Đội ngũ CineTick xin cảm ơn!</div>
        </div>
        ...
        document.getElementById('download-qr-link').addEventListener('click', function(e) {
            ...
            var imageUrl = "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data={{ $hoaDon->ID_HoaDon ?? '---' }}";
            ...
            fetch(imageUrl, {mode: 'cors'})
                ...
                }).catch(() => alert('Không thể tự động tải về QR code. Vui lòng click chuột phải vào ảnh để lưu.'));
        });
        </script>
        @else
    ...
    <div class="total-section @if(session('status') !== 'success') failure @endif">
        <div class="total-row">
            <span class="total-label @if(session('status') !== 'success') failure @endif">Tổng cộng</span>
            <span class="total-amount @if(session('status') !== 'success') failure @endif">
                {{ isset($hoaDon->TongTien) ? number_format($hoaDon->TongTien, 0, ',', '.') . 'đ' : ($totalPrice ?? '---') }}
            </span>
        </div>
    </div>
